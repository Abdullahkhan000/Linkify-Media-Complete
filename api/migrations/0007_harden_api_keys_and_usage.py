...
def hash_existing_keys(apps, schema_editor):
    ...
    for api_key in APIKey.objects.all().iterator():
        ...
        tier = (
            Profile.objects.filter(user_id=api_key.user_id)
        ...
        api_key.scopes = (
            ["search", "usage"]
            + (["batch"] if tier in {"pro", "business"} else [])
        )

        api_key.save(
            update_fields=["key_hash", "prefix", "scopes"]
        )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        (
            "api",
            "0006_supportconversation_supportticket_conversation_and_more",
        ),
    ]
