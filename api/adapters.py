...
class CustomAccountAdapter(DefaultAccountAdapter):
    def save_user(self, request, user, form, commit=True):
        ...
        pending_name = request.session.pop("pending_signup_name", "").strip()
        request.session.pop("pending_signup_email", None)
        if pending_name:
            first_name, _, last_name = pending_name.partition(" ")
    ...
    def send_mail(self, template_prefix, email, context):
        ...
        try:
            ...
            if not settings.DEBUG:
                raise
            logger.warning("Resend delivery failed in development: %s", exc)
            EmailMultiAlternatives(
                subject=subject,
                body=body_text,
                from_email=from_email,
